var app = angular.module('myapp',[]);
	app.controller('showController',function($scope,$timeout,$http){
		...
		var init_check_isright = $http({
			method: 'GET',
			url: 'http://117.78.32.164:8080/demo-1.0/rest/showcase/getSoftwareInfo'
		});
		...
		$scope.changePage = function(name){
			...
			$timeout(function(){
				if(name === 'software'){
					...
					var check_isright = $http({
						method: 'GET',
						url: 'http://117.78.32.164:8080/demo-1.0/rest/showcase/getSchoolInfo'
					});
					check_isright.then(function(resp){
						$scope.line = {
							'one':resp.data[0].value1,
					...
					var check_isright = $http({
						method: 'GET',
						url: 'http://117.78.32.164:8080/demo-1.0/rest/showcase/getEnterprisesInfo'
					});
					check_isright.then(function(resp){
						$scope.line = {
							'one':resp.data[0].value1,
					...
					var check_isright = $http({
						method: 'GET',
						url: 'http://117.78.32.164:8080/demo-1.0/rest/showcase/getEpibolyInfo'
					});
					check_isright.then(function(resp){
						$scope.line = {
							'one':resp.data[0].value1,
